...
async fn test_session_lifecycle() {
    ...
    let session = auth_manager::tokens::session::create(&db, "user-123", device_info, 3600).unwrap();
...
async fn test_multiple_sessions_per_resource() {
    ...
    let device_info = auth_manager::storage::models::DeviceInfo::default();
    let s1 = auth_manager::tokens::session::create(&db, "user-456", device_info.clone(), 3600).unwrap();
    let s2 = auth_manager::tokens::session::create(&db, "user-456", device_info.clone(), 3600).unwrap();
    let s3 = auth_manager::tokens::session::create(&db, "user-789", device_info.clone(), 3600).unwrap();
